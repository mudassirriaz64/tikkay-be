...
export const getContactPageData = asyncHandler(async (_req: Request, res: Response) => {
...
export const getSubmissions = asyncHandler(async (_req: AuthRequest, res: Response) => {
...
export const markSubmissionRead = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
...
export const deleteSubmission = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
...
const createGenericCrud = <T extends Document>(Model: Model<T>, name: string) => ({
...
export const contactMethods = {
  ...contactMethodsCRUD,
  saveAll: asyncHandler(async (req: AuthRequest, res: Response) => {
    const rawMethods = Array.isArray(req.body) ? req.body : req.body.methods || [];
    await ContactMethod.deleteMany({});
    const itemsToInsert = rawMethods.map((m: any, idx: number) => ({
      icon: m.icon || 'phone',
...
export const openingDays = {
  ...
  saveAll: asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const itemsToInsert = rawDays.map((d: any, idx: number) => ({
      day: d.day || 'Monday',
      hours: d.hours || '',
