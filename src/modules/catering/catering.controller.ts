...
import { CateringRequest, CateringStatus } from './catering.model';
import { AuthRequest } from '../../middleware/auth.middleware';
import { Types } from 'mongoose';

export const createCateringRequest = asyncHandler(async (req: AuthRequest, res: Response) => {
  const {
    user_id,
    event_type,
    guest_count,
    ...
    estimated_total = 0,
    is_live_tandoor_requested = false,
  } = req.body;
...
export const getMyCateringRequests = asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const getAllCateringRequests = asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const updateCateringStatus = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const { status, coordinator_notes } = req.body as { status: CateringStatus; coordinator_notes?: string };

  const validStatuses: CateringStatus[] = ['inquiry', 'under-review', 'confirmed', 'completed', 'cancelled'];
