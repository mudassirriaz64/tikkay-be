...
import { JobRole, JobApplication, ApplicationStatus } from './careers.model';
...
const INITIAL_JOBS = [
  {
    ...
    description:
      'Master the iron skewers over red-hot charcoal embers. Responsible for marinade consistency, fire temperature control, and delivering legendary smoky tikkas.',
    requirements: [
  ...
  },
  {
    ...
    location: 'DHA Phase 5, Lahore',
    description:
      'Expertise in slapping garlic, roghani, and whole wheat naans into clay tandoors at high peak volumes.',
    requirements: [
  ...
  },
  {
    ...
    description:
      'Coordinate corporate feasts, wedding live stations, and outdoor gala catering from booking to live fire execution.',
    requirements: [
  ...
  },
  {
    ...
    description:
      'Warmly welcome guests, manage seating and digital queue management, and ensure the authentic Tikkay Shikkay hospitality.',
    requirements: [
...
export const getOpenJobs = asyncHandler(async (_req: Request, res: Response) => {
...
export const getAllJobsAdmin = asyncHandler(async (_req: AuthRequest, res: Response) => {
  await ensureSeedJobs();
...
export const updateApplicationStatus = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const validStatuses: ApplicationStatus[] = ['applied', 'reviewed', 'interview', 'hired', 'rejected'];
