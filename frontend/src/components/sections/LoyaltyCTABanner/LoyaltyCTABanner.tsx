...
import { JoinLoyaltyModal } from "@/components/sections/accounts/JoinLoyaltyModal";

export function LoyaltyCTABanner() {
  const router = useRouter();
  const { isSignedIn, profile } = useAccount();
  const [memberCount, setMemberCount] = useState<number>(12470);
              ...
              <div>
                ...
                <div className="mt-8">
                  <MagneticButton
                    ...
                    className="px-6 py-3 rounded-xl font-medium bg-[#D9381E] text-white hover:bg-[#b82d16] cursor-pointer"
                  >
                    {profile?.is_loyalty_member ? "View Loyalty Dashboard" : "Join WhatsApp Club"}
                  </MagneticButton>
                </div>
              </div>
