...
import { CareersHeroData } from "@/types";
  ...
  useGSAP(
    () => {
      if (reducedMotion) {
        ...
        gsap.set(".careers-hero-ember", { opacity: 0 });
        return;
      }
  ...
  return (
    <section ref={ref} className="relative overflow-hidden bg-[var(--bg-deep)]">
      <div className="absolute inset-0 scale-110">
        <Image
          src={data.imageUrl}
          alt="Team working at the charcoal grill"
          fill
          priority
          sizes="100vw"
          className="object-cover object-center"
        />
        <div className="careers-hero-scrim absolute inset-0 bg-[var(--bg-deep)]/70" />
        <div className="careers-hero-scrim absolute inset-0 bg-gradient-to-b from-[var(--bg-deep)]/70 via-transparent to-[var(--bg-base)]" />
      </div>

      <div
        aria-hidden="true"
        className="careers-hero-smoke-a absolute -left-32 top-1/4 h-[420px] w-[420px] rounded-full bg-[var(--accent-ember)]/10 opacity-[0.08] blur-[120px]"
      />
      <div
        aria-hidden="true"
        className="careers-hero-smoke-b absolute -right-24 bottom-0 h-[360px] w-[360px] rounded-full bg-[var(--accent-peach)]/10 opacity-[0.06] blur-[120px]"
      />
      ...
      {embers.map((ember, index) => (
        <span
          key={index}
          aria-hidden="true"
          className={`careers-hero-ember absolute rounded-full ${ember.className}`}
        />
      ))}
      ...
      <div className="relative mx-auto flex min-h-[62vh] w-full max-w-[1280px] flex-col justify-center px-4 pb-16 pt-24 md:min-h-[70vh] lg:px-[64px]">
        <div className="max-w-4xl space-y-7">
          <span className="careers-hero-eyebrow inline-flex items-center gap-3 text-xs font-bold uppercase tracking-[0.24em] text-[var(--accent-peach)]">
          ...
          <h1 className="font-[family:var(--font-serif)] text-4xl font-bold uppercase leading-[0.95] tracking-[-0.02em] text-[var(--text-primary)] md:text-7xl lg:text-[96px]">
            <span className="careers-hero-line block overflow-hidden pb-[0.12em] -mb-[0.12em]">
          ...
          <p className="careers-hero-subtitle max-w-xl text-base leading-relaxed text-[var(--text-body)] md:text-lg">
            {data.description}
          </p>
        </div>
      </div>
